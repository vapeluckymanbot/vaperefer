...
BOT_TOKEN = "5317577616:AAFqSZV4FgMClakwYTCm91rBU6Yp6fQOkwM"
PAYMENT_CHANNEL = "@vapepayment" #add payment channel here including the '@' sign
OWNER_ID = 621784715 #write owner's user id here.. get it from @MissRose_Bot by /id
CHANNELS = ["@indianmviehunt"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
...
def send_text(message):
   try:
    ...
    if message.text == "⚙️ Set Wallet":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_⚠️Send your Paytm wallet Number._",
...
def trx_address(message):
   try:
    ...
    if len(message.text) == 10:
        ...
        bot.send_message(message.chat.id, "*💹Your Paytm wallet Number set to " +
                         data['wallet'][user]+"*", parse_mode="Markdown")
        json.dump(data, open('users.json', 'w'))
        return menu(message.chat.id)
    else:
        bot.send_message(
            message.chat.id, "*⚠️ It's Not a Valid Paytm wallet Number!*", parse_mode="Markdown")
        return menu(message.chat.id)
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
